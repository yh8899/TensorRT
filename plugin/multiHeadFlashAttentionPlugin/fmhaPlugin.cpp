    ...
    int32_t pos, PluginTensorDesc const* inOut, int32_t nbInputs, int32_t nbOutputs) noexcept
{
    ...
    try
    {
        ...
        case 1: res = inOut[pos].type == DataType::kHALF && inOut[pos].dims.nbDims == 4; break;
        case 2: res = inOut[pos].type == DataType::kHALF && inOut[pos].dims.nbDims == 4; break;
        case 3: res = inOut[pos].type == inOut[0].type && inOut[pos].dims.nbDims == 4; break;
...
{
    // input[ 0]:  [float16],  (b, s, h, d)
    // input[ 1]:  [float16],  (b, s, h, d)
    // input[ 2]:  [float16],  (b, s, h, d)
    ...
    try
    {
        ...
        // PLUGIN_VALIDATE(mSM);
        // PLUGIN_VALIDATE(mSoftmaxLse);

        // update cuseqlens when bs or seq changed.
        int32_t const batchSize = inputDesc[0].dims.d[0];
        int32_t const seqLen_q = inputDesc[0].dims.d[1];
        int32_t const seqLen_k = inputDesc[1].dims.d[1];
        int32_t const head_num = inputDesc[0].dims.d[2];
        int32_t const head_num_k = inputDesc[1].dims.d[2];
        int32_t const head_size = inputDesc[0].dims.d[4];

        void* cudaMem{nullptr};
        PLUGIN_CHECK(cudaMalloc(&cudaMem, sizeof(float) * (batchSize * head_num * seqLen_q)));
        bert::make_cuda_shared(mSoftmaxLse, cudaMem);
